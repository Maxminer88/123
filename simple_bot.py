...
class TelegramBot:
    ...
    def _question_timer(self, chat_id, time_limit):
        ...
        if chat_id in user_states and user_states[chat_id].get('mode') == 'quiz' and not user_states[chat_id].get('answered', False):
            # Time's up
    ...
    def check_quiz_answer(self, chat_id, username, user_answer):
        ...
        response_time = round(current_time - start_time, 1)
        time_limit = user_states[chat_id]['current_question']['time_limit']

        # Check if answer was given in time
    ...
    def show_calculator(self, chat_id):
        ...
        calc_text = (
            ...
            "Пример: /calc inflation 100 105\n"
            "(расчет инфляции от 100 до 105)"
        )
